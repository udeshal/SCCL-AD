...
class TabNeutralAD(nn.Module):
    def __init__(self, model, x_dim,config):
        ...
        # commented for ablation self.enc,self.trans,self.scales = model._make_nets(x_dim,config)
        # 
        self.enc,self.trans = model._make_nets(x_dim,config)
        #print('Scales :', self.scales);
        #print('Transformer :', self.trans);
    ...
    def forward(self,x):
        ...
        for i in range(self.num_trans): 
            # 
            mask = self.trans[i](x)           
            #commented for ablation scaled = self.scales[i](x)            
            #print('Scaled : ', scaled.shape)
            #commented for ablation mask = self.trans[i](scaled)
